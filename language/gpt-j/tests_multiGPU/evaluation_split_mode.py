...
def get_args():
    ...
    parser.add_argument("--test_execution_date", required=True,
...
def main():
    ...
    root_logdir = '../../logs/qgpt-j/Offline'
    default_log_name="mlperf_log_accuracy.json"
    results_path_list = []
    for idx in range(args.num_splits):
        results_path_list.append(os.path.join(root_logdir, f"{args.test_execution_date}_{args.dataset_file.split('/')[-1].split('.')[0]}_{idx}"))
